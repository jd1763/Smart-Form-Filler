...
def node_eval(js_snippet):
    ...
    if 'inferNamePartFromLabel' in js_snippet:
        ...
        if "first" in lab:
            return "firstName"
        if "last" in lab or "surname" in lab or "family" in lab:
            return "lastName"
        return None

    if 'inferAddressPartFromLabel' in js_snippet:
        lab = re.search(r'\("([^"]+)"\)', js_snippet).group(1).lower()
        if "address" in lab or "line 1" in lab:
            return "street"
        if "city" in lab or "town" in lab:
            return "city"
        if "state" in lab or "province" in lab or "region" in lab:
            return "state"
        if "zip" in lab or "postal" in lab or "postcode" in lab:
            return "zip"
        if "country" in lab or "nation" in lab:
            return "country"
        return None
