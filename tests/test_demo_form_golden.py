...
import os, json, pathlib, re, pytest
...
_CANON = {
    ...
    "dob":"dob","birthdate":"dob","dateofbirth":"dob",
    "gender":"gender",

    # address
    "address":"street","address1":"street","addressline1":"street","streetaddress":"street","street":"street",
    "city":"city","citytown":"city","town":"city",
    "state":"state","province":"state","region":"state","stateprovinceregion":"state",
    "zip":"zip","zipcode":"zip","zipcodepostalcode":"zip",
    "postal":"zip","postalcode":"zip","postcode":"zip",
    "countrynation":"country","country":"country",

    # links
    "linkedinprofileurl":"linkedin","linkedin":"linkedin",
    "github":"github","portfolio":"github",
    "personalwebsite":"website","website":"website",

    # education
    "highestdegree":"highestDegree","degree":"highestDegree",
    "university":"university","college":"university","institute":"university","universitycollegeinstitute":"university",
    "fieldofstudy":"fieldOfStudy","major":"fieldOfStudy",
    "yearofgraduation":"graduationYear","graduationyear":"graduationYear",

    # experience
    "companyname":"company","employername":"company","company":"company","employer":"company",
    "yourjobtitle":"jobTitle","officialjobtitle":"jobTitle","positiontitle":"jobTitle","jobtitle":"jobTitle","title":"jobTitle",
    "startdate":"start_date","start_date":"start_date","employmentstart":"start_date",
    "enddate":"end_date","end_date":"end_date","employmentenddate":"end_date",
    "roledescriptionkeyresponsibilitiesduties":"roleDescription",
    "jobdutiesworkresponsibilities":"roleDescription","responsibilities":"roleDescription","duties":"roleDescription",
    "organization":"previousEmployer","previousemployer":"previousEmployer",

    # eligibility / files
    "areyouauthorizedtoworkintheus":"workAuthorization","workauthorization":"workAuthorization",
    "willyounoworinthefuturerequiresponsorship":"requiresSponsorship","requiressponsorship":"requiresSponsorship",
    "uploadresumeresumefilecvupload":"resumeFile","coverletteradditionalinformation":"coverLetter","coverletter":"coverLetter",
}
...
def _guess_from_label(label: str) -> str | None:
    s = (label or "").lower()
    if "first" in s: return "firstName"
    if "surname" in s or "last" in s or "family" in s: return "lastName"
    if "email" in s: return "email"
    if "mobile" in s or "phone" in s: return "phoneNumber"
    if "birth" in s or "dob" in s: return "dob"
    if "gender" in s: return "gender"
    if ("address" in s and "line" in s) or "street" in s: return "street"
    if "city" in s or "town" in s: return "city"
    if "state" in s or "province" in s or "region" in s: return "state"
    if "zip" in s or "postal" in s or "postcode" in s: return "zip"
    if "country" in s or "nation" in s: return "country"
    if "linkedin" in s: return "linkedin"
    if "github" in s or "portfolio" in s or "website" in s: return "github"
    if "company" in s or "employer" in s or "organization" in s: return "company"
    if "title" in s or "position" in s: return "jobTitle"
    if "start" in s: return "start_date"
    if "end" in s: return "end_date"
    if "role" in s or "duties" in s or "responsibilit" in s: return "roleDescription"
    if "authorized to work" in s or "authorization" in s: return "workAuthorization"
    if "sponsorship" in s: return "requiresSponsorship"
    return None
...
def predict_labels(labels):
    ...
    if isinstance(raw, list) and raw:
        for i, item in enumerate(raw):
            ...
            if isinstance(item, dict) and "topk" in item and isinstance(item["topk"], list) and item["topk"]:
                first = item["topk"][0]
    ...
    for lab, p in zip(labels, preds):
        if p is None:
            fixed.append(None)
            continue
        if re.search(r"[ /]|address|name|email|phone|zip|postal|state|city|country|degree|university|title|employer|company", str(p), flags=re.I):
...
def test_resolver_exposes_all_needed_keys():
    ...
    needed = {"firstName","lastName","email","phoneNumber","street","city","state","zip","country"}
